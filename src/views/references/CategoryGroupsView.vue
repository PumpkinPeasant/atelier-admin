...
import { referencesApi, type CategoryGroup } from '@/services/references.mock'
import type { TableHeader } from '@/interfaces/ui/table.interface'

const notifications = useNotificationsStore()

const headers: TableHeader<CategoryGroup>[] = [
...
const { items, totalCount, currentPage, pageSize, sortData, isLoading, loadItems, search } =
  useDataTable<CategoryGroup>(referencesApi.listCategoryGroups)
...
<template>
  <section class="card">
    <DataTableHeader
      ...
      create-btn-text="Добавить группу"
    ...
    <DataTable
      ...
      :headers="headers"
      :items="items"
      empty-state-title="Группы не найдены"
    >
      <template #actions>
