...
import { referencesApi, type Material } from '@/services/references.mock'
import type { TableHeader } from '@/interfaces/ui/table.interface'

const notifications = useNotificationsStore()

const headers: TableHeader<Material>[] = [
  { key: 'id', title: 'ID', sortable: true },
  { key: 'name', title: 'Название', sortable: true },
  { key: 'composition', title: 'Состав', sortable: true },
]

const { items, totalCount, currentPage, pageSize, sortData, isLoading, loadItems, search } =
  useDataTable<Material>(referencesApi.listMaterials)
...
<template>
  <section class="card">
    <DataTableHeader
      search-placeholder="Поиск по материалам"
      create-btn-text="Добавить материал"
    ...
    <DataTable
      ...
      :headers="headers"
      :items="items"
      empty-state-title="Материалы не найдены"
    >
      <template #actions>
