...
import { referencesApi, type Size } from '@/services/references.mock'
import type { TableHeader } from '@/interfaces/ui/table.interface'

const notifications = useNotificationsStore()

const headers: TableHeader<Size>[] = [
  { key: 'id', title: 'ID', sortable: true },
  { key: 'label', title: 'Размер', sortable: true },
  { key: 'sort_order', title: 'Порядок', sortable: true },
]

const { items, totalCount, currentPage, pageSize, sortData, isLoading, loadItems, search } =
  useDataTable<Size>(referencesApi.listSizes)
...
<template>
  <section class="card">
    <DataTableHeader
      search-placeholder="Поиск по размерам"
      create-btn-text="Добавить размер"
    ...
    <DataTable
      ...
      :headers="headers"
      :items="items"
      empty-state-title="Размеры не найдены"
    >
      <template #actions>
